...
| 드롭 구독자 | 파괴 이벤트를 받아 정의의 기본 수량으로 파편 생성. 업그레이드 보정은 6에서 연결 | 타일 제거 재실행, 무기 코드 의존 |
...
## 4. 변경 계획 — 5A/5B 구현 완료, 후속 검증과 3B/6/7
...
- 초기 스테이지 한 곳에서 시작 공간·귀환 동선·필수 자원을 새 정의로 배치한다. 광물 분포를 의도적으로 설계한다.
...
구현 완료: **5A → 5B**. 후속 순서: **3B → 6 → 7 → 8**. 4R의 현 구조를 재사용하고 5A/5B의 남은 수동 검증은 통합 검증에서 확인한다.
