# 2026-09-12 재구성 이전 계획 및 구현 이력

이 문서는 재구성 직전 원문을 보존한 역사 기록이다. 아래의 과거 계획과 다음 단계 인계 지시는 현재 작업 지시가 아니다. 이후 작업은 같은 폴더의 2026-09-10-demo-revision-plan.md를 따른다. 검증 결과는 각 기록 작성 당시의 보고이며 2026-09-12 계획 재구성에서 재실행하지 않았다.

---

# 데모 이후 UI·채굴 전환 작업 계획

### 2026-09-12 — 4단계 완료: 채굴 데이터와 셀 파괴 기능

- 수정: `MiningDefinition`(최대 HP, 드롭 자원, 기본 수량, 파괴 가능 여부)과 정의를 참조하는 `MiningLogicalTile`을 추가했다. `Assets/Data/Definitions/Mining` 및 `Assets/Data/StageMaps/MiningTiles`에 기존 8개 자원 각각의 정의·논리 타일을 생성했다. 공통 `PlatformLogicalTile`은 그대로 보호 지형이다.
- 런타임: `StageMap`이 Platform 셀 좌표별 현재 HP를 소유하고 `TryGetMiningCell`/`TryApplyMiningDamage`를 제공한다. 공유 Tile·정의 에셋에는 런타임 상태를 기록하지 않는다. 파괴 시 Logic+Visual 셀을 함께 제거하고 3x3 AutoTile 갱신, `TilemapCollider2D.ProcessTilemapChanges`, 물리 동기화 및 셀·월드 위치·정의를 담은 단일 `MiningCellDestroyed` 이벤트를 처리한다. 저장 데이터나 드롭 생성에는 연결하지 않았다.
- 편집기: Stage Map Placement에 명시적 `Paint Content: Mining`과 `Mining Logical Tile` 선택을 추가했다. Platform에서만 기존 연결 지형의 논리 타일을 광종 타일로 바꾸며 Visual은 유지한다. 기존 일반 그리기·채우기·Auto Texture는 광종을 덮어쓰지 않고, StageMap 설정/변환도 광종 논리 타일을 보존한다.
- 검증: Unity 6000.5.1f1 Editor 재컴파일 성공(ready/비컴파일), 콘솔 오류 0개를 확인했다. `StageMapMiningSmokeTest`를 Editor에서 실행해 독립 HP, 정의 공유, 보호벽, Logic·Visual 동시 제거, 중복 파괴 이벤트 방지, 새 StageMap 인스턴스의 초기 HP를 검증했다. 8개 정의+8개 논리 타일을 AssetDatabase로 재로드해 모두 `TryValidate` 통과했고, `git diff --check`도 통과했다. 등록된 Unity EditMode 테스트는 0개다.
- 미검증: 실제 PlayMode에서 레이저가 셀 좌표를 전달하는 경로, 화면 AutoTile 결과, CompositeCollider 충돌 갱신, 실제 씬 언로드→재진입은 5단계 이후의 통합 PlayMode 검증이 필요하다. 이 단계에서는 스테이지에 광종을 배치하지 않았고 Floatage/드롭/업그레이드/저장 경로도 변경하지 않았다.
- 다음 단계 인계: 5단계에서 `PlasmaGunController`의 첫 충돌을 `StageMap`+셀 좌표로 판정하고 `TryApplyMiningDamage` 및 `MiningCellDestroyed`를 연결한다. 파괴 이벤트의 드롭 구독과 자원 회수는 5B에서만 추가한다. 현재 보호 Platform, 세션 한정 파괴 상태, StageMap 편집기의 Mining 모드는 유지하고 6~7단계의 업그레이드 이전·실제 스테이지 배치·Floatage 제거로 범위를 넓히지 않는다.
